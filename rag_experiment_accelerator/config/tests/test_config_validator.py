...
import requests
from rag_experiment_accelerator.config.config_validator import fetch_json_schema, fetch_json_schema_from_url, fetch_json_schema_from_file
...
@patch('builtins.open', new_callable=mock_open, read_data='{"$schema": "http://test.com/schema.json"}')
def test_fetch_json_schema_from_file_returns_json_from_file(mock_open, mock_isfile):
    ...
    mock_open.assert_called_once_with('example.schema.json', 'r', encoding='utf8')
...
def test_fetch_json_schema_from_file_uses_a_relative_path(mock_open, mock_isfile):
    ...
    cwd = os.getcwd()
    schema_file_path = os.path.join(cwd, 'example.schema.json')
    source_file_path = os.path.join(os.path.basename(cwd), 'source_file.json')
...
def test_fetch_json_schema_updates_cache(mock_fetch_json_schema_from_url, mock_schema_cache):
    schema_reference = 'http://test.com/schema.json'
    schema_cache = {}

    mock_schema_cache.__contains__.return_value = False
    mock_schema_cache.__setitem__.side_effect = schema_cache.__setitem__
    mock_fetch_json_schema_from_url.return_value = {'$schema': 'http://test.com/schema.json'}

    fetch_json_schema(schema_reference, 'source_file.json')
    assert schema_cache == {schema_reference: {'$schema': 'http://test.com/schema.json'}}


@patch('rag_experiment_accelerator.config.config_validator.schema_cache')
@patch('rag_experiment_accelerator.config.config_validator.fetch_json_schema_from_url')
def test_fetch_json_schema_returns_from_url_when_http(mock_fetch_json_schema_from_url, mock_schema_cache):
    schema_reference = 'http://test.com/schema.json'
...
@patch('rag_experiment_accelerator.config.config_validator.schema_cache')
@patch('rag_experiment_accelerator.config.config_validator.fetch_json_schema_from_url')
def test_fetch_json_schema_returns_from_url_when_https(mock_fetch_json_schema_from_url, mock_schema_cache):
...
def test_fetch_json_schema_returns_from_file(mock_fetch_json_schema_from_file, mock_schema_cache):
    schema_reference = './schema.json'

    mock_schema_cache.__contains__.return_value = False
    mock_fetch_json_schema_from_file.return_value = {'$schema': 'http://test.com/schema.json'}

    fetch_json_schema(schema_reference, 'source_file.json')
    mock_fetch_json_schema_from_file.assert_called_once_with(schema_reference, 'source_file.json')

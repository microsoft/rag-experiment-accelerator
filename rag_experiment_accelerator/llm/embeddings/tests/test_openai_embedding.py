...
@patch(
    "rag_experiment_accelerator.llm.embeddings.openai_embedding.openai.Embedding.create"
)
def test_generate_embedding(create_embedding_mock):
    ...
    creds = OpenAICredentials(
        "open_ai", "openai_api_key", "openai_api_version", "openai_endpoint"
    )
...
def test_emebdding_dimension_has_default():
    creds = OpenAICredentials(
        "azure", "openai_api_key", "openai_api_version", "openai_endpoint"
    )
    model = OpenAIEmbeddingModel("text-embedding-ada-002", creds)

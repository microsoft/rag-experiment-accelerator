...
documents = ["This is a test.", "C'est un test.",
             "Dies ist ein Test.", "Questa è una prova."]
...
def test_detect_language():
    with patch(
        ...
        primary_language = language_evaluator.detect_language(
            "This is a test.")
...
def test_detect_languages():
    with patch(
        ...
        response = language_evaluator.detect_language(
            documents).return_value = detect_languages_response
        ...
        for i, doc in enumerate(detect_languages_response):
            ...
            assert doc["confidence_score"] == response[i].get(
                'confidence_score')
...
def test_is_language_match():
    with patch.object(LanguageEvaluator, "detect_language", create=True
        ...
        language_evaluator.is_language_match.assert_called_with(
            "C'est un test.", "fr")

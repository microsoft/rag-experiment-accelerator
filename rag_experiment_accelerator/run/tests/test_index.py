...
class TestIndex(unittest.TestCase):
    ...
    def run_test(
        ...
        # Arrange
        mock_generate_index_name.return_value = "test_index_name"
        ...
        mock_Config.return_value.MAX_WORKER_THREADS = 1
        ...
        mock_Config.return_value.AzureSearchCredentials.AZURE_SEARCH_SERVICE_ENDPOINT = (
            "test_endpoint"
        )
        mock_Config.return_value.AzureSearchCredentials.AZURE_SEARCH_ADMIN_KEY = (
            "test_key"
        )
        mock_Config.return_value.AZURE_OAI_CHAT_DEPLOYMENT_NAME = "test_deployment_name"
        doc1 = MagicMock()
        doc1 = {"key1": {"content": "content1", "metadata": {"source": "source1"}}}
        doc2 = MagicMock()
        doc2 = {"key2": {"content": "content2", "metadata": {"source": "source2"}}}
        mock_load_documents.return_value = [doc1, doc2]

        # Mock the generate_embedding method for each embedding model
        for model in embedding_models:
            model.generate_embedding = MagicMock(return_value="embedding_value")
        ...
        chunks = [
            {
                ...
                "content": "content1",
                "filename": "source1",
                "source_display_name": "source1",
                "content_vector": embedding_models[0].generate_embedding()[0],
            },
            {
                "id": "key2",
                "content": "content2",
                "filename": "source2",
                "source_display_name": "source2",
                "content_vector": embedding_models[1].generate_embedding()[0],
            },
        ]
        ...
        expected_first_call_args = [
            ...
            "test_index_name",
            "test_key",
            embedding_models[0].generate_embedding(),
        ...
        mock_generate_index_name.assert_called()
        mock_create_acs_index.assert_called()

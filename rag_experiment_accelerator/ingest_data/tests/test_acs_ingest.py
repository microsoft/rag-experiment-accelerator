...
class TestGenerateTitle(unittest.TestCase):
    def test_my_hash_with_string(self):
        # Arrange
        test_string = "Hello, World!"
        expected_hash = "65a8e27d8879283831b664bd8b7f0ad4"  # Precomputed MD5 hash of "Hello, World!"
    ...
    def test_my_hash_with_empty_string(self):
        # Arrange
        test_string = ""
        expected_hash = "d41d8cd98f00b204e9800998ecf8427e"  # Precomputed MD5 hash of an empty string
    ...
    def test_my_hash_with_numbers(self):
        # Arrange
    ...
    def test_generate_title(self, MockResponseGenerator):
        # Arrange
        ...
        MockResponseGenerator.return_value = mock_response_generator
    ...
    @patch("rag_experiment_accelerator.ingest_data.acs_ingest.ResponseGenerator")
    def test_generate_summary(self, MockResponseGenerator):
        # Arrange
    ...
    @patch("rag_experiment_accelerator.ingest_data.acs_ingest.logger")
    def test_generate_qna(self, mock_logger, mock_json_loads, MockResponseGenerator):
        # Arrange
        mock_docs = [
            Mock(page_content="This is a test document content."),
            Mock(page_content=""),
        ]
        mock_deployment_name = "TestDeployment"
        ...
        mock_response_generator.generate_response.return_value = mock_response
        MockResponseGenerator.return_value = mock_response_generator
        mock_json_loads.return_value = Mock(
            dict(user_prompt="Test question?", output_prompt="Test answer.")
        )
        ...
        self.assertEqual(result.iloc[0]["user_prompt"], "Test question?")
        self.assertEqual(result.iloc[0]["output_prompt"], "Test answer.")
        self.assertEqual(result.iloc[0]["context"], "This is a test document content.")
        MockResponseGenerator.assert_called_once_with(
            deployment_name=mock_deployment_name
        )
    ...
    @patch("rag_experiment_accelerator.ingest_data.acs_ingest.json.loads")
    @patch("rag_experiment_accelerator.ingest_data.acs_ingest.logger")
    def test_generate_qna_with_invalid_json(
        self, mock_logger, mock_json_loads, MockResponseGenerator
    ):
        # Arrange
        mock_docs = [
            Mock(

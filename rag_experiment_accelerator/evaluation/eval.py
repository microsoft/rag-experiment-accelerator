...
def bleu(predictions, references):
    ...
    results = bleu.compute(predictions=predictions,
                           references=references, max_order=2)
...
def llm_answer_relevance(question, answer):
    ...
    """
    config = Config()
    result = ResponseGenerator(deployment_name=config.AZURE_OAI_EVAL_DEPLOYMENT_NAME).generate_response(
...
def plot_apk_scores(df, run_id, client):
    fig = px.line(df, x="k", y="score",
                  title="AP@k scores", color="search_type")
...
def plot_mapk_scores(df, run_id, client):
    fig = px.line(df, x="k", y="map_at_k",
                  title="MAP@k scores", color="search_type")
    plot_name = "mean_average_precision_at_k.html"
    client.log_figure(run_id, fig, plot_name)


def plot_map_scores(df, run_id, client):
    fig = px.bar(df, x="search_type", y="mean",
                 title="MAP scores", color="search_type")
...
def evaluate_prompts(
    ...
    try:
        ...
        logger.error(
            f"Unable to create the '{eval_score_folder}' directory. Please ensure you have the proper permissions and try again")
        raise e

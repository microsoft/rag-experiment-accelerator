...
def llm_answer_relevance(question, answer):
    ...
    """
    config = Config()
    result = ResponseGenerator().generate_response(sys_message=llm_answer_relevance_instruction, prompt=answer, engine_model=config.EVAL_MODEL_NAME, temperature=config.TEMPERATURE)
...
def llm_context_precision(question, context):
    ...
    config = Config()
    prompt = "\nquestion: " + question + "\ncontext: " + context + "\nanswer: "
    result = ResponseGenerator().generate_response(sys_message=llm_context_precision_instruction, prompt=prompt, engine_model=config.EVAL_MODEL_NAME, temperature=config.TEMPERATURE)
